...
from _cookies import set_cookies, check_cookies
...
def _login(secret_credentials_path, cookie_name, cookie_key, cookie_expiry_days, app_url, color, justify_content):
    if check_cookies(cookie_name, cookie_key):
        return
    
    check_authentification(cookie_name, cookie_key, cookie_expiry_days, app_url)
...
def check_authentification(cookie_name, cookie_key, cookie_expiry_days, app_url):
    auth_code = st.query_params.get("code")
    st.query_params.clear()
    flow = google_auth_oauthlib.flow.Flow.from_client_secrets_file(
        "authentification/google_credentials.json", # replace with you json credentials from your google auth app
        scopes=["openid", "https://www.googleapis.com/auth/userinfo.profile", "https://www.googleapis.com/auth/userinfo.email"],
        redirect_uri=app_url,
    )
    if auth_code:
